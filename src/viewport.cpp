...
{ // Needed for geometry changes
  ...
  {
    rectCon.disconnect();
    rectCon = rect->valueChanged.connect(sigc::mem_fun(this, &ViewportPrivate::onRectChange));
  }
...
: SceneElement(*shState->screen()), sceneLink(this)
{
...
void Viewport::composite()
{
  ...
  Scene::composite();
  // If any effects are visible, request parent Scene to render them.
  if (renderEffect)
