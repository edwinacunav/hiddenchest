...
static size_t
strcpySafe(char *dst, const char *src, size_t dstMax, int srcN)
{
  if (srcN < 0) srcN = strlen(src);
...
static void
initReadOps(PHYSFS_File *handle, SDL_RWops &ops, bool freeOnClose)
{
...
// Basically just a case-insensitive search for the folder "Fonts"...
static PHYSFS_EnumerateCallbackResult
...
struct OpenReadEnumData
{
  ...
  const char *filename;
  size_t filenameN;
  // Optional hash to translate full filepaths (used with path cache)
