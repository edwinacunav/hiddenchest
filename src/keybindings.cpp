...
static bool verifyDesc(const BindingDesc &desc)
{
  const Input::ButtonCode codes[] =
  {
      ...
      Input::L, Input::R,
      Input::Shift, Input::Ctrl, Input::Alt/*, Input::Backspace, Input::Tab,
      Input::F3, Input::F4, Input::F5, Input::F6, Input::F7, Input::F8, Input::F9, Input::F10, Input::F11*/
  };
  elementsN(codes);
  size_t i;
  for (i = 0; i < codesN; ++i)
    if (desc.target == codes[i]) break;
  if (i == codesN) return false;
...
static bool readBindings(BDescVec &out, const std::string &dir, uint32_t rgssVersion)
{
  ...
  if (hd.formVer != FORMAT_VER)  return false;
  if (hd.rgssVer != rgssVersion) return false;
  /* Arbitrary max value */
  if (hd.count > 1024) return false;
  out.resize(hd.count);
  ...
  for (size_t i = 0; i < hd.count; ++i)
    if (!verifyDesc(out[i])) return false;
  return true;
}
...
BDescVec loadBindings(const Config &conf)
{
  ...
  if (readBindings(d, conf.commonDataPath, conf.rgssVersion)) return d;
