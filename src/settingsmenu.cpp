...
std::string sourceDescString(const SourceDesc &src)
{
  ...
  case JHat:
    ...
    snprintf(buf, sizeof(buf), "Hat %d:%c", src.d.jh.hat, pos);
    return buf;
  case JAxis:
    snprintf(buf, sizeof(buf), "Axis %d%c", src.d.ja.axis, src.d.ja.dir == Negative ? '-' : '+');
    return buf;
  }
  assert(!"unreachable");
  return "";
...
struct Widget
{ // Widgets have a static size and position, defined at creation
  Widget(SMP *p, const IntRect &rect);
  // Public methods take coordinates in global window coordinates
...
protected:
  SMP *p;
  IntRect rect;
  // Protected abstract methods are called with widget-local coordinates
...
struct BindingWidget : Widget
{
  ...
  // Source slots
  SourceDesc src[4];
  // Flag indicating whether a slot source is used for multiple button targets (red indicator)
  bool dupFlag[4];
...
protected:
  ...
  // Get the slot cell index that contains (x,y), or -1 if none
...
struct Button : Widget
{
  ...
  const char *str;
  Callback cb;
  Button(SMP *p, const IntRect &rect, const char *str, Callback cb)
  : Widget(p, rect), str(str), cb(cb), hovered(false)
  {}
...
bool SettingsMenu::onEvent(const SDL_Event &event)
{
	...
	case SDL_KEYDOWN:
		...
		// Don't let the user bind keys that trigger this engine functions
